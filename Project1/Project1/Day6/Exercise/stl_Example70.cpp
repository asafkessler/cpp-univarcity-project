...
int main() {
  ...
  names.push_back("Ido");
  names.push_back("Tal");
  names.push_back("Moshe");
  names.push_back("Yossi");
  names.push_back("David");

  string someString = "Yossi";
  ...
  vector<string>::iterator findIt = find(names.begin(), names.end(), "Yossi");
  cout << "Find algorithm" << endl << endl;
  if(findIt == names.end()) 
    cout << "Not found!!" << endl;
  else 
     cout << "First object with value Yossi found at offset "
       << (findIt-names.begin()) << endl;



  int n = count(names.begin(), names.end(), "David");  
  cout << "The name David appears " << n << " times in the vector." << endl;
